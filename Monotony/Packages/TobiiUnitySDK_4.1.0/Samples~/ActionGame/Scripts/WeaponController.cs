...
namespace Tobii.Gaming.Examples.ActionGame
{
	...
	public class WeaponController : MonoBehaviour
	{
		...
		public Vector3 AimedAtLocation
		{
			get
			{
				return IsShootingAtGaze
					? WorldGazeCrosshairTransformProjected.position
					: WorldCenterCrosshairTransformProjected.position;
			}
		}
		...
		private void Update()
		{
			...
			if (IsAiming)
			{
				MainCamera.fieldOfView =
					Mathf.Lerp(MainCamera.fieldOfView, AimedFieldOfView, Time.unscaledDeltaTime * 5);
			}
			else
			{
				MainCamera.fieldOfView =
					Mathf.Lerp(MainCamera.fieldOfView, NormalFieldOfView, Time.unscaledDeltaTime * 5);
			}
					...
					if (gazePoint.IsRecent())
					{
						...
						else
						{
							var viewportCoordinates =
								ExtendedView.CameraWithExtendedView.WorldToViewportPoint(
									WorldCenterCrosshairTransformProjected.position);
							var normalizedCoordinatesDelta = (gazePoint.Viewport -
							                                  new Vector2(viewportCoordinates.x,
								                                  viewportCoordinates.y)) * 2;
		...
		private void CalculateWorldGazeCrosshairPosition()
		{
			...
			RaycastHit hitInfo;
			var direction = WorldGazeCrosshairTransformFixed.position -
			                ExtendedView.CameraWithExtendedView.transform.position;
			direction.Normalize();
			if (Physics.Raycast(ExtendedView.CameraWithExtendedView.transform.position, direction, out hitInfo,
				MaxProjectionDistance, RaycastLayerMask))
			{
		...
		{
			_screenCenterCrosshairPosition =
				ExtendedView.CameraWithExtendedView.WorldToScreenPoint(WorldCenterCrosshairTransformProjected.position);
			//Clamp to pixel
			_screenCenterCrosshairPosition = new Vector2(Mathf.FloorToInt(_screenCenterCrosshairPosition.x + 0.25f),
				Mathf.FloorToInt(_screenCenterCrosshairPosition.y + 0.25f));
		}
		...
		private void CalculateWorldCenterCrosshairPosition()
		{
			...
			if (Physics.Raycast(ExtendedView.CameraWithoutExtendedView.transform.position,
				ExtendedView.CameraWithoutExtendedView.transform.forward, out hitInfo, MaxProjectionDistance,
				RaycastLayerMask))
			{
			...
			{
				WorldCenterCrosshairTransformProjected.position =
					ExtendedView.CameraWithoutExtendedView.transform.position +
		...
		private void FindHitDataForPotentialOverride()
		{
			...
			direction.Normalize();
			if (Physics.Raycast(OptionalWeaponFireOriginOverride.position, direction, out hitInfo,
				MaxProjectionDistance, RaycastLayerMask))
			{
				WeaponHitData = hitInfo;
				IsWeaponHitObject = true;
		...
		private Vector3 ScreenToWorldProjection(Vector2 screenPosition)
		{
			...
			var worldPositionFixed =
				ExtendedView.CameraWithExtendedView.ScreenToWorldPoint(new Vector3(screenPosition.x, screenPosition.y,
					100));
		...
		private void UpdateAimBlockedCrosshair()
		{
			if (AimBlockedCrosshairImage != null)
			{
				    ...
				    && (Vector3.Distance(AimedAtLocation, WeaponHitData.point) > 0.01f))
				{
					...
					AimBlockedCrosshairImage.rectTransform.anchoredPosition =
						new Vector2(screenPosition.x - Screen.width / 2.0f, screenPosition.y - Screen.height / 2.0f);
				}
				else
				{
