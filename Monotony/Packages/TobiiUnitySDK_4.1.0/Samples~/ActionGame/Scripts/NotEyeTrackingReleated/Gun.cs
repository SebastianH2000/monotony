    ...
    public abstract class Gun : MonoBehaviour
    {
            ...
            if (_animationTime < FireAnimationRotationCurve[FireAnimationRotationCurve.length - 1].time)
            {
                ...
                _animationTransform.localRotation = _baseRotation *
                                                    Quaternion.Euler(0.0f,
                                                        FireAnimationRotationCurve.Evaluate(_animationTime), 0.0f);
            }
            else
            {
                _animationTransform.localRotation = _baseRotation;
        ...
        private void Fire(bool shootAtGaze)
        {
            ...
            for (var i = 1; i < BulletsPerShot; i++)
            {
                ...
                var left = Vector3.Cross(mainDirection,
                    Vector3.Dot(mainDirection, Vector3.up) > 0.95 ? Vector3.right : Vector3.up);
                var up = Vector3.Cross(mainDirection, left);
                var direction = mainDirection + rand.x * left + rand.y * up;
                RaycastHit hitInfo;
                if (Physics.Raycast(origin, direction, out hitInfo, WeaponController.MaxProjectionDistance,
                    WeaponController.RaycastLayerMask))
                {
                    ShootBullet(hitInfo);
                }
            }
        ...
        private void SpawnBulletHole(RaycastHit hitInfo, Transform hitObject)
        {
            ...
            var bulletHole =
                (GameObject) Instantiate(BulletHolePrefab, hitInfo.point + hitInfo.normal * 0.0001f, hitRotation);
            if (hitObject != null)
            {
