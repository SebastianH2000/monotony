...
namespace Tobii.Gaming.Examples.ActionGame
{
	...
	{
		[Tooltip(
			"Time to hold the interaction button before an object gets selected. Set to 0 to respond directly on button clicks.")]
		...
		private const int
			RaycastLayerMask = ~0x24; //0b100100;                  // ignore "ignore raycast" and "ui" layers

		private readonly float FovealAngle = 2.0f; // slight overestimate of the foveal angle

		private readonly float
			GazeOriginToScreenDistanceInInches = 24f; // estimate of a typical viewing distance using eye tracking
		...
		void OnGUI()
		{
			if (DebugCrosshairOverrideZone)
			{
				...
				var buttonPosition = new Vector2(crosshairPosition.x - OverrideZoneRadiusInPixels,
					crosshairPosition.y - OverrideZoneRadiusInPixels);
				var buttonSize = new Vector2(OverrideZoneRadiusInPixels * 2, OverrideZoneRadiusInPixels * 2);
				GUI.Button(new Rect(buttonPosition.x, buttonPosition.y, buttonSize.x, buttonSize.y),
					CrosshairOverrideZoneCircle);
			}
		}
		...
		private void UpdateObjectFocusedForSelection()
		{
			...
			if (newlyFocusedObject == null)
			{
				...
				if (newlyFocusedObject != null)
				{
					if (Vector3.Distance(newlyFocusedObject.transform.position,
						ExtendedView.CameraWithoutExtendedView.transform.position) > MaxInteractionDistance)
					{
		...
		private void UpdatePreviouslyFocusedObject(GameObject newlyFocusedObject, GameObject recentlyFocusedObject,
			float recentlyFocusedLastSeen)
		{
		...
		private void OutlineObjects(Color outlineColor)
		{
			...
			HighlightsPostEffect.SetHighlightedObjects(ObjectsToHighlight(_focusedObject, _previouslyFocusedObject,
				_previouslyLastSeen));
		}

		private GameObjectAndOpacity[] ObjectsToHighlight(GameObject objectToHighlight,
			GameObject objectWithFadedHighlight, float fadedObjectLastSeen)
		{
			return new[]
			{
